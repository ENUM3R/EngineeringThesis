...
export const CustomToolbar = ({ label, onNavigate, onView, currentView, setCurrentView  }) => (
    <div className="rbc-toolbar" style={{ display: "flex", justifyContent: "space-between", marginBottom: "10px" }}>
      ...
      <span className="rbc-toolbar-label" style={{ fontWeight: "bold" }}>{label}</span>
      <div>
        {["quarter", "month", "week", "day", "agenda"].map(view => (
          <button
            key={view}
            onClick={() => { onView(view); setCurrentView(view); }}
            style={{
              marginLeft: "5px",
